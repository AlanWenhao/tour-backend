...
const articles = 
    ...
    category_name VARCHAR(64) NOT NULL COMMENT '文章类别',
    title VARCHAR(128) NOT NULL COMMENT '文章题目',
    author VARCHAR(64) NOT NULL COMMENT '文章作者',
    poster VARCHAR(256) NOT NULL COMMENT '文章海报',
...
const comments =
    ...
    child TEXT NOT NULL COMMENT '回复内容',
    PRIMARY KEY(id)
    );`;
...
// 设为管理员
exports.setAdmin = (id) => {
    let _sql = `update users set is_admin=1 where id=${id};`;
    return query(_sql);
}
exports.cancelAdmin = (id) => {
    let _sql = `update users set is_admin=0 where id=${id};`;
    return query(_sql);
}

// 添加类别
exports.insertCategory = (value) => {
    let _sql = "insert into category set name=?,picture=?;"
    return query(_sql, value)
}
...
exports.insertArticle = (value) => {
    let _sql = "insert into articles set category_id=?,category_name=?,title=?,poster=?,author=?,summary=?,content=?,moment=?,pv=?,thumb=?;"
    return query(_sql, value)
}
...
// 根据类目删除文章
exports.deleteArticleByCate = (cateId) => {
    let _sql = `delete from articles where category_id=${cateId}`
    return query(_sql);
}
...
// 获取【人们文章，点赞最多】
exports.queryHotArticles = (value) => {
    let _sql = `select * from articles order by thumb desc limit 0,5;`
    return query(_sql, value);
}
...
exports.addComment = (value) => {
    let _sql = "insert into comments set username=?,avatar=?,article_id=?,content=?,moment=?,child=?;";
    return query(_sql, value);
}
// 查询评论
exports.getCommentsByid = (id) => {
    let _sql = `select * from comments where article_id=${id};`;
    return query(_sql);
}
