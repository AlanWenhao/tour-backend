...
exports.queryArticles = (value) => {
    let _sql = `select * from articles where username=? order by id desc limit ?,?;`
    return query(_sql, value);
}
// 分页获取文章 
exports.queryAllArticles = (value) => {
    let _sql = `select * from articles order by id desc limit ?,?;`
    return query(_sql, value);
}
exports.queryArticleNum = (username) => {
    let _sql;
    if (username) _sql = `select count(*) as count from articles where author="${username}";`
    else _sql = `select count(*) as count from articles;`
